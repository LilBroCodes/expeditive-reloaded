...
public class DyableShapedRecipeSerializer implements RecipeSerializer<DyableShapedRecipe> {
    ...
    @Override
    public DyableShapedRecipe read(Identifier id, JsonObject json) {
        ...
        Map<String, Ingredient> key = readSymbols(JsonHelper.getObject(json, "key"));
        String[] pattern = removePadding(getPattern(JsonHelper.getArray(json, "pattern")));

        int width = pattern[0].length();
        int height = pattern.length;

        DefaultedList<Ingredient> inputs = createPatternMatrix(pattern, key, width, height);
    ...
    @Override
    public DyableShapedRecipe read(Identifier id, PacketByteBuf buf) {
    ...
    @Override
    public void write(PacketByteBuf buf, DyableShapedRecipe recipe) {
